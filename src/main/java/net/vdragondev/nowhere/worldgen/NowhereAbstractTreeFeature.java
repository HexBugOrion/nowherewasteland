...
import net.vdragondev.nowhere.worldgen.generators.SaplingData;
import net.vdragondev.nowhere.worldgen.noise.FastNoise;
import net.vdragondev.nowhere.worldgen.tags.FabricTags;
...
public abstract class NowhereAbstractTreeFeature<TFC extends NowhereTreeConfig> extends Feature<TFC> {

    protected static FastNoise fastNoise;
    protected long seed;

    public NowhereAbstractTreeFeature(Codec<TFC> configCodec) {
        super(configCodec);
    }
    ...
    public void placeTrunk(NowhereTreeConfig config, Random random, Set<BlockPos> blockSet, StructureWorldAccess reader, BlockPos pos, BlockBox boundingBox) {
        if (canLogPlaceHere(reader, pos)) {
            this.setFinalBlockState(blockSet, reader, pos, config.getTrunkProvider().getBlockState(random, pos), boundingBox);
        }
    }

    public void placeBranch(NowhereTreeConfig config, Random random, Set<BlockPos> blockSet, StructureWorldAccess reader, BlockPos pos, BlockBox boundingBox) {
        if (canLogPlaceHere(reader, pos)) {
            this.setFinalBlockState(blockSet, reader, pos, config.getTrunkProvider().getBlockState(random, pos), boundingBox);
        }
    }

    public void placeLeaves(NowhereTreeConfig config, Random random, Set<BlockPos> blockSet, StructureWorldAccess reader, BlockPos pos, BlockBox boundingBox) {
        if (isAir(reader, pos)) {
    ...
    public void placeLeaves(NowhereTreeConfig config, Random random, StructureWorldAccess reader, int x, int y, int z, BlockBox boundingBox, Set<BlockPos> blockPos) {
    ...
    public void placeNetherTrunk(NowhereTreeConfig config, Random random, Set<BlockPos> blockSet, StructureWorldAccess reader, BlockPos pos, BlockBox boundingBox) {
        if (canLogPlaceHereNether(reader, pos)) {
            this.setFinalBlockState(blockSet, reader, pos, config.getTrunkProvider().getBlockState(random, pos), boundingBox);
        }
    }

    public void placeNetherBranch(NowhereTreeConfig config, Random random, Set<BlockPos> blockSet, StructureWorldAccess reader, BlockPos pos, BlockBox boundingBox) {
        if (canLogPlaceHereNether(reader, pos)) {
            this.setFinalBlockState(blockSet, reader, pos, config.getTrunkProvider().getBlockState(random, pos), boundingBox);
        }
    }
     ...
     */
    public static boolean isDesiredGroundwDirtTag(TestableWorld reader, BlockPos pos, NowhereTreeConfig config) {
        return reader.testBlockState(pos, (state) -> {
            Block block = state.getBlock();
    ...
    public static boolean isDesiredGroundwNetherTags(TestableWorld reader, BlockPos pos, NowhereTreeConfig config) {
        return reader.testBlockState(pos, (state) -> {
            Block block = state.getBlock();
            for (Block block1 : config.getWhitelist()) {
    ...
    public static boolean isDesiredGroundwEndTags(TestableWorld reader, BlockPos pos, NowhereTreeConfig config) {
        return reader.testBlockState(pos, (state) -> {
            Block block = state.getBlock();
            for (Block block1 : config.getWhitelist()) {
     ...
     */
    public static boolean isDesiredGroundwSandTag(TestableWorld reader, BlockPos pos, NowhereTreeConfig config) {
        return reader.testBlockState(pos, (state) -> {
            Block block = state.getBlock();
            for (Block block1 : config.getWhitelist()) {
    ...
    public void buildTrunkBase(Set<BlockPos> treeBlocksSet, TestableWorld reader, NowhereTreeConfig config, Random rand, BlockBox boundingBox, BlockPos... trunkPositions) {
     ...
     */
    public void setSoil(Set<BlockPos> treeBlocksSet, TestableWorld reader, NowhereTreeConfig config, Random rand, BlockBox boundingBox, BlockPos... trunkPositions) {
        if (trunkPositions.length > 0) {
            BlockPos.Mutable mutableTrunk = new BlockPos.Mutable();
            for (BlockPos trunkPos : trunkPositions) {
                mutableTrunk.set(trunkPos);
    ...
    public void setDisk(StructureWorldAccess world, Random random, BlockPos pos, NowhereTreeConfig config) {
